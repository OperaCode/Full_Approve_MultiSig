...
const eventIcons: Record<string, React.ReactNode> = {
  ...
  BudgetReleased: <Banknote className="h-3.5 w-3.5 text-accent" />,
};
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-6">
      <div className="mb-4 flex items-center gap-2">
        ...
        {events.length > 0 && (
          <span className="ml-auto rounded-full bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary tabular-nums">
            {events.length}
          </span>
        )}
      </div>
      ...
      {events.length === 0 ? (
        <div className="flex flex-col items-center py-8 text-center">
          <div className="mb-2 h-2 w-2 rounded-full bg-primary/40 animate-node-pulse" />
        ...
        <div className="max-h-80 space-y-2 overflow-y-auto pr-1">
          {events.map((event, i) => (
            <div
              ...
              className="flex items-start gap-3 rounded-lg bg-secondary/50 p-3 animate-block-stack"
